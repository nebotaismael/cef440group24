...
class FirebaseService {
  ...
  // Course Management (Enhanced with proper model support)
  ...
  // Enhanced Enrollment Management with proper model support
  static Future<String> createEnrollment(Enrollment enrollment) async {
    final docRef = await enrollmentsCollection.add(enrollment.toFirestore());
    return docRef.id;
  }
  ...
  static Future<void> enrollStudent(String studentId, String courseId) async {
    ...
    // Create new enrollment with proper model
    ...
    await enrollmentsCollection.add(enrollment.toFirestore());
  }
  ...
  static Future<List<Enrollment>> getEnrollmentsByCourse(String courseId) async {
  ...
  static Future<List<Enrollment>> getEnrollmentsByStudent(String studentId) async {
    final snapshot = await enrollmentsCollection
        .where('studentId', isEqualTo: studentId)
        .get();
    return snapshot.docs.map((doc) => Enrollment.fromFirestore(doc)).toList();
  ...
  static Future<List<String>> getEnrolledStudentIds(String courseId) async {
    final snapshot = await enrollmentsCollection
        .where('courseId', isEqualTo: courseId)
        .where('status', isEqualTo: EnrollmentStatus.active.toString())
        .get();
  ...
  // Get enrolled students with full user data
  static Future<List<app_user.User>> getEnrolledStudents(String courseId) async {
    final enrollmentIds = await getEnrolledStudentIds(courseId);
    final List<app_user.User> students = [];

    for (final studentId in enrollmentIds) {
  ...
  // Get student's enrolled courses with full course data
  static Future<List<Course>> getStudentCourses(String studentId) async {
    final courseIds = await getStudentCourseIds(studentId);
  ...
  static Future<bool> isStudentEnrolled(String studentId, String courseId) async {
    final snapshot = await enrollmentsCollection
        .where('studentId', isEqualTo: studentId)
        .where('courseId', isEqualTo: courseId)
        .where('status', isEqualTo: EnrollmentStatus.active.toString())
  ...
  // Auto-migration method to fix enrollment data consistency
  static Future<void> migrateEnrollmentData() async {
    try {
      print('Starting enrollment data migration...');

      // Get all enrollments
      final enrollmentSnapshot = await enrollmentsCollection.get();

      for (final doc in enrollmentSnapshot.docs) {
        final data = doc.data();

        // Check if enrollment has missing fields and update them
        if (!data.containsKey('status')) {
          await doc.reference.update({
            'status': EnrollmentStatus.active.toString(),
            'createdAt': data['enrolledAt'] ?? FieldValue.serverTimestamp(),
            'updatedAt': FieldValue.serverTimestamp(),
          });
        }

        // Verify that the referenced course and student exist
        final courseExists = await getCourseById(data['courseId']) != null;
        final studentExists = await getUserById(data['studentId']) != null;

        if (!courseExists || !studentExists) {
          print('Removing orphaned enrollment: ${doc.id}');
          await doc.reference.delete();
        }
      }

      print('Enrollment data migration completed successfully');
    } catch (e) {
      print('Error during enrollment migration: $e');
    }
  }
