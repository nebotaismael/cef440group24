...
from config import allowed_file, token_required, secure_save_file, cleanup_files, read_image, detect_faces, \
    extract_face_features, verify_location, generate_attendance_id, detector_backend

# Global variable for detector (imported from config)
# detector_backend = "retinaface"  # Now imported from config
...
def login():
    ...
    try:
        # Query users by email since document IDs are Firebase UIDs
        users_query = db.collection('users').where('email', '==', auth.username).limit(1)
        ...
        user_id = user_doc.id  # Get the actual Firebase UID
        
        # Check password using matriculeOrStaffId
        if 'matriculeOrStaffId' not in user_data or auth.password != user_data['matriculeOrStaffId']:
            logger.warning(f"Failed login attempt for user: {auth.username}")
            return jsonify({'error': 'Invalid credentials'}), 401

        # Generate token
        token_expiry = datetime.utcnow() + timedelta(seconds=Config.JWT_EXPIRATION)
        token = jwt.encode(
            {
                'id': user_id,  # Use Firebase UID instead of username
                'email': user_data.get('email', ''),
                'name': user_data.get('fullName', ''),
                'role': user_data.get('role', ''),
                'matriculeOrStaffId': user_data.get('matriculeOrStaffId', ''),
                'exp': token_expiry
            },
            Config.JWT_SECRET,
            algorithm="HS256"
...
def register_face():
    ...
    try:
        ...
        try:
            ...
            return jsonify({'error': 'Liveness check failed due to technical error'}), 500
...
def verify_attendance():
    ...
    # Validate user role - only students should mark attendance
    user_role = request.user.get('role', '')
    if user_role not in ['student', 'UserRole.student']:
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Liveness check failed during verification: {e}")
            return jsonify({'error': 'Liveness check failed due to technical error'}), 500
        ...
        try:
            ...
            # Check if session is active (you might have status field)
            if session_data.get('status') != 'active':
                return jsonify({'error': 'Session is not active'}), 400
            
            # Get course ID from session to validate enrollment
            ...
            if course_id:
                ...
                enrollments_query = db.collection('enrollments')\
                    ...
                    .where('status', '==', 'active')\
                    .limit(1)
                enrollments = enrollments_query.get()
        ...
        # Store record in Firestore (use 'attendance' collection to match database)
