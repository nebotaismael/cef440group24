...
from livenesschech import Config, logger, face_mesh, detector_backend
...
# Face detection with MediaPipe (use shared instance from livenesschech)
...
def extract_face_features(image, face_rect):
    ...
    face_img = image[y1_padded:y2_padded, x1_padded:x2_padded]
    
    # Validate extracted face image
    if face_img.size == 0:
        logger.warning("Extracted face image is empty")
        # Return original face area without padding as fallback
